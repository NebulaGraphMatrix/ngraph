...
//
// cluster coefficient of an UNDIRECTED GRAPH where only (i,j) is stored where
// i<j.
//

double cluster_coeff_undirected(const Graph &A, const Graph::vertex &a) {
  const Graph::vertex_set neighbors = A.out_neighbors(a) + A.in_neighbors(a);
  int n = neighbors.size();
...
double cluster_coeff_undirected(const Graph &A, Graph::const_iterator t) {
  const Graph::vertex_set neighbors =
...
class cluster_coeff_table {
 ...
 public:
  ...
  double &operator[](const Graph::vertex &a) { return coeff_[a]; }
  ...
  double update(const Graph::edge &e) {
    ...
    //
    Graph::vertex_set v = (G_.in_neighbors(a) * G_.in_neighbors(b));
    v.insert(a);
    // v.insert(b);
  ...
  double update_undirected(const Graph::edge &e) {
    ...
    Graph::vertex_set a_neighbors = (G_.in_neighbors(a) + G_.out_neighbors(a));
    Graph::vertex_set b_neighbors = (G_.in_neighbors(b) + G_.out_neighbors(b));
