...
class SCC {
 private:
  ...
  void tarjan_(Ge v) {
    ...
    S_.push(v);
    SS_.insert(v);
    for (vn_iter p = G_.out_neighbors_begin(v); p != G_.out_neighbors_end(v);
         p++) {
 ...
 public:
  SCC(const Graph &G)
      : G_(G),
        S_(),
        SS_(),
        LowLink_(G.num_vertices()),
        Index_(G.num_vertices()),
        index_(1),
        SCC_count_(0),
        SCC_index_(G.num_vertices()) {
    for (Graph::const_iterator v = G.begin(); v != G.end(); v++) {
      if (Index_[Graph::node(v)] == 0) tarjan_(Graph::node(v));
    }
  }

  UInt component(Ge v) const { return SCC_index_[v]; }
};
