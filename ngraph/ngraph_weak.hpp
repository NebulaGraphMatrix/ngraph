...
void weak_components_increment(const Graph &G, Equivalence &E,
                               std::set<Graph::vertex> &C, Node_Iter begin,
                               Node_Iter end) {
  C.insert(begin, end);
  for (Node_Iter pv = begin; pv != end; pv++) {
    ...
    for (Graph::const_vertex_iterator p = G.in_begin(gv); p != G.in_end(gv);
         p++) {
      Graph::vertex e = Graph::node(p);
      if (includes_elm(C, e)) E.insert(e, v);
    }
    for (Graph::const_vertex_iterator p = G.out_begin(gv); p != G.out_end(gv);
         p++) {
      Graph::vertex e = Graph::node(p);
      if (includes_elm(C, e)) E.insert(v, e);
    }
