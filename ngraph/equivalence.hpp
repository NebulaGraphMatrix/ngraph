...
class equivalence {
 ...
 public:
  ...
  equivalence(void)
      : E_(), S_(), equivalence_class_num_(0), recording_(false), E1_(), M_(){};

  static const element_set& collection(const_iterator p) { return p->second; }

  static index_t index(const_iterator p) { return p->first; }

  unsigned int num_classes() const { return S_.size(); }
  ...
  unsigned int num_elements() const { return E_.size(); }

  bool is_recording() const { return recording_; }

  void recording_on() { recording_ = true; }

  void recording_off() { recording_ = false; }
  ...
  const vector<triplet>& merge_list() const { return M_; }

  const map<T, index_t>& original_class_indices() const { return E1_; }
  ...
  bool includes(const T& a) const { return (index(a) != 0); }
  ...
  void insert(const T& a, const T& b) {
    ...
    if (E_a != 0)  // if a is already in an equivalence class,
    {
          ...
          if (E_b != E_a)  // but different equivalent classes
      {
        ...
        unsigned int bigger_class_index = (sa.size() > sb.size() ? E_a : E_b);
        unsigned int smaller_class_index = (sa.size() > sb.size() ? E_b : E_a);
        ...
        if (recording_)
          M_.push_back(make_triplet(smaller_class_index, bigger_class_index,
                                    bigger_class_index));
