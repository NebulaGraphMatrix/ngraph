...
int main(int argc, char *argv[]) {
  ...
  for (DegreeList::const_iterator p = D.begin(); p != D.end(); p++) {
    ...
    for (equivalence<Graph::vertex>::const_iterator p = Q.begin(); p != Q.end();
         p++) {
    ...
    cout << degree << " " << num_nontrivial_clusters << " "
         << size_nontrivial_clusters << " " << Max_size << " "
         << Q.num_elements();
    ...
    if (print_distribution) {
      ...
      for (map<unsigned int, unsigned int>::const_iterator f = F.begin();
        ...
        if (print_clustering) cout << ", " << S[f->first] / (f->second);
        cout << ") ";
      }
    }
