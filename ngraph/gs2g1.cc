...
/*
    convert a general (string) graph file into a contigous 1-based
    graph.
...
int main(int argc, char *argv[]) {
  ...
  if (argc > 1) perform_mapping_only = ("-t" == string(argv[1]));

  Graph G;

  typedef std::string input_graph_type;
  ...
  while (getline(std::cin, input_line)) {
    ...
    if (!perform_mapping_only) G.insert_edge(ifrom, ito);
  }
