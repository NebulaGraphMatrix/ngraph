...
* / namespace NGraph {
  ...
  class static_tGraph {
   ...
   public:
    ...
    unsinged int in_degree(unsigned int i) const {
      return in_edges_rows_[i + 1] - in_edges_rows_[i];
    }
    unsinged int degree(unsigned int i) const {
      return nodes_[i + 1] - nodes_[i];
    }
