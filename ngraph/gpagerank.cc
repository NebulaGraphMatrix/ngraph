...
int main(int argc, char *argv[]) {
  ...
  if (argc > 2) max_delta = atof(argv[2]);

  sGraph G;
  cin >> G;

  uInt iterations_used = 0;
  map<sGraph::vertex, double> P =
      pagerank(G, iterations_used, max_iterations, max_delta);

  cerr << "iterations = " << iterations_used << "  max_delta = " << max_delta
       << "\n";
  for (map<sGraph::vertex, double>::const_iterator i = P.begin(); i != P.end();
       i++) {
